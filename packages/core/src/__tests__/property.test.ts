...
describe("qquotes Property-based Testing", () => {
    ...
    const quoteArb = fc.record({
        ...
        tags: fc.array(
            fc.string({ minLength: 2, maxLength: 10, alpha: true }),
            { maxLength: 5 },
        ),
        metadata: fc.record({
    ...
    it("should always return a quote from random() if the store is non-empty", () => {
        fc.assert(
            fc.property(
                fc.array(quoteArb, { minLength: 1, maxLength: 50 }),
                (quotes) => {
                    // Ensure unique IDs for initialization
                    const uniqueQuotes = Array.from(
                        new Map(quotes.map((q) => [q.id, q])).values(),
                    );
    ...
    it("should always find a quote if searching for its exact text", () => {
        fc.assert(
            fc.property(
                fc.array(quoteArb, { minLength: 1, maxLength: 10 }),
                (quotes) => {
                    const uniqueQuotes = Array.from(
                        new Map(quotes.map((q) => [q.id, q])).values(),
                    );
